...
mount_one (const char *spec, const char *node, char *type, const char *opts,
  ...
  if (status == EX_TIVO) 
  {
    // EX_TIVO means the mount failed due to problem with the host, i.e. dns
    // fail or permission denied, we should try to find an alias. Normal error
    // output has been suppressed, aliasing must be invisible.
    char *desired=xstrdup(spec);  // remember the desired mount path
    if (!strstr(desired,"://"))   // host://path is bogus, avoid matching the default
    {
      if (verbose) fprintf(stderr, "mount: %s failed, searching nfsaliases...\n", desired);
      FILE *aliases=fopen("/devbin/nfsaliases", "r");
      if (!aliases) 
      {
        if (verbose) fprintf(stderr, "mount: /devbin/nfsaliases - %s\n", strerror(errno));
      } else    
      {
        char line[256];
        ...
        {
          char *left, *right, *s;
          for (s=line; *s && *s != '#'; s++); *s=0; // strip comment
          // Pick out "left" and "right" space-delimited tokens (extraneous crap at EOL will be ignored)
          left=strtok_r(line, " \f\t\v\r\n", &s);
          right=strtok_r(NULL, " \f\t\v\r\n", &s);
          if (left && right && !strncmp(desired, left, strlen(left)))
          {
            // Here, the desired nfs path starts with the left token, replace it with right  
            const char *tail=desired+strlen(left);       
            char *alias=xmalloc(strlen(right)+strlen(tail)+1);
            strcpy(alias,right);
            strcat(alias,tail);
            if (*(tail-1)==':') 
            {
              // The left token ends with ':', so this is a hostname alias.  The
              // resulting string becomes the desired path for all subsequent
              // matches.
              if (verbose) fprintf(stderr, "mount: changing %s to %s\n", desired, alias);
              free(desired);
              desired=alias;
            } else
            {
              // The left token doesn't end with ':', so this is a pathname
              // alias, try to mount the result.  
              if (verbose) fprintf(stderr, "mount: aliasing %s to %s\n", desired, alias);
              status = try_mount_one (alias, node, type, opts, freq, pass, 0, 0);
              free(alias);
              if (status != EX_TIVO) goto out;
            }  
          }
        }
      }    
    } 
    // Aliases failed, issue generic error using the original pathname.
