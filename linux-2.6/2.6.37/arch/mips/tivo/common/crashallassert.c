 ...
 *
 * Copyright (C) 2005, 2006, 2013 TiVo Inc. All Rights Reserved.
 */
...
void backtrace_current(void *unused)
{
    thread_fbuf *exec_cntxt = (thread_fbuf *)current->exec_cntxt;

	printk("CPU %d PID %d (%s):\n", 
           smp_processor_id(),
           current->pid, 
           (exec_cntxt ? ( exec_cntxt->cntxt? 
                             exec_cntxt->cntxt->name:current->comm):
                             current->comm));
