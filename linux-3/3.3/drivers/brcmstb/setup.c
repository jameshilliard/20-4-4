...
#else
        // this had to be deferred until after boardID is known
        if (boardID & kTivoConfigGen10Strap_IsBridge)
        {
            printk ("Configuring RGMII phy\n");
            genet_pdata[0].phy_type = BRCM_PHY_TYPE_EXT_RGMII;
            genet_pdata[0].phy_id = BRCM_PHY_ID_NONE;
            genet_pdata[0].phy_speed=1000;
        } else
        {
            printk("Configuring internal phy\n");
...
/* Pace - removed this define, because it introduces the need for the generic SPI driver       */
/*        the config option needs to be enabled elsewhere, because flash support depends on it */
#undef CONFIG_BRCM_HAS_SPI
#endif
...
#ifdef CONFIG_PACESTB
/* Brutal reset */
	printk("Pace AON reset\n");
	BDEV_UNSET(BCHP_GIO_AON_IODIR_LO, 1 << 18);   /* It's an output now */
	BDEV_SET(BCHP_GIO_AON_DATA_LO, 1 << 18);      /* And drive it low to invoke the reset  */
