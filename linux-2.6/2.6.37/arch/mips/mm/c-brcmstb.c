...
int brcm_cacheflush(unsigned long addr, unsigned long bytes,
	unsigned int cache)
{
#if !defined(CONFIG_BRCM_ZSCM_L2) && (!defined(CONFIG_PACESTB) || !defined(CONFIG_BRCM_SCM_L2))
...
void brcm_inv_prefetch(unsigned long addr, unsigned long size)
{
	...
	DEV_SET_RB(BMIPS_GET_CBR() + BMIPS_RAC_CONFIG, 0x100);
#elif defined(CONFIG_PACESTB) && defined(CONFIG_MTI_R5K)
	unsigned long flags;
...
#elif defined(CONFIG_BRCM_ZSCM_L2) || (defined(CONFIG_PACESTB) && defined(CONFIG_BRCM_SCM_L2))
...
void brcm_get_cache_info(struct brcm_cache_info *info)
{
	...
	(defined(CONFIG_PACESTB) && defined(CONFIG_MTI_R5K))
...
{
    switch (routine) {
    case '0': // bcm_inv_rac_all
        break;
    case '1': // r4k_flush_cache_all();
        break;
    case '4': // r4k_flush_cache_mm(current->mm);
        break;
    case '9': // r4k_flush_dcache_all();
        break;
    case 'A': // r4k_flush_icache_all();
        break;
    case 'C': // r4k___flush_cache_all();
        r4k___flush_cache_all();
        break;
    }
}
