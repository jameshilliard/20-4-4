...
static int proc_usb_test(struct dev_state *ps, void *arg)
{
   ...
   DBGTEST(("USB_TEST: --> Port %d, action %d, response=%u, root hub ptr = 0x%08x", 
      TestInfo.port_num, TestInfo.action, TestInfo.response, (unsigned int)pRootHub));
   ...
   case TEST_POWER_OFF:
      ...
      */
      ...
      /* It seems on bcm7406 (Seattle) to be useful to clear this status bit
         in the OHCI hub */
      ret = usb_control_msg(pRootHub, usb_sndctrlpipe(pRootHub, 0),
               USB_REQ_CLEAR_FEATURE, USB_RT_PORT, USB_PORT_FEAT_C_CONNECTION, 
               port_num + 1, NULL, 0, HZ);
      if (ret < 0) {
         err("HUB_PORT: --> %s (%d) failed (err = %d)", __FUNCTION__, pDevice->devnum, ret);
      }
   ...
   case TEST_VENDOR_ID:
      if (pDevice)
         response =  (pDevice->descriptor.idVendor << 16)
                   | (pDevice->descriptor.idProduct);
      else   
         response = 0;
      break;
#if 0
   case TEST_DEVICE_MS:
      ...
      if((pDevice->config->interface->altsetting->bInterfaceClass == 8)
       &&(pDevice->config->interface[0]->altsetting->bInterfaceSubClass == 6))
         response = 1;
      else
         response = 0;
      break;
   ...
   case TEST_MODE:
      /* The USB 2.0 specification stipulates that the most significant
         byte (bits 15..8) of the wIndex field is the selector
         identifying the specific test mode, and the least significant
         byte (bits 7..0) identify the port number. TestInfo.response is
         used to pass the required test mode to this function. Construct the
         combined test mode and port number */
      wIndex = (TestInfo.response << 8) + (port_num + 1);

      DBGTEST(("TEST MODE: wIndex = %d",wIndex));

      /* Reset the port if test mode is zero */
      if (TestInfo.response == 0)
      {
         DBGTEST(("TEST MODE: Resetting"));
         ...
         if (portsts)
         {
            ...
            if (ret >= 0)
            {
               /* The ehci_hub_control() function in ehci-hub.c states that
                  we must wait for at least 50 ms and then read the status to
                  bring the port out of reset. However, this doesn't seem
                  to work. Currently, the only way to bring the port out of
                  test mode is to reboot the STB */
               mdelay(100);
               ret = usb_control_msg(pRootHub, usb_rcvctrlpipe(pRootHub, 0),
                     USB_REQ_GET_STATUS, USB_DIR_IN | USB_RT_PORT, 0, 
                     wIndex, portsts, sizeof(struct usb_hub_status), HZ);
            }
      ...
      {
         DBGTEST(("TEST MODE: Not reset - doing something else "));
   ...
   DBGTEST(("USB_TEST: --> The response is %d return %d", TestInfo.response, ret));
