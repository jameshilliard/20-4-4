...
static void config_allocate( void )
{
     ...
     #ifdef TIVO
     {
          const char* tivoRoot = getenv( "TIVO_ROOT" );
          if ((tivoRoot != NULL) && (*tivoRoot != 0) &&
              (strcmp( tivoRoot, "/" ) != 0))
          {
               char tivoRootedModuleDir[ PATH_MAX ];
               (void) snprintf( tivoRootedModuleDir, sizeof(tivoRootedModuleDir),
                   "%s%s", tivoRoot, MODULEDIR );
               tivoRootedModuleDir[ PATH_MAX - 1 ] = 0;

               dfb_config_set( "module-dir", tivoRootedModuleDir );
          }
     }
     #endif
     ...
     #ifdef TIVO
     {
         // This should really be guarded with #if BOOTARCH == X86,
         // but we don't have the compile flags set up to have the defines
         // passed in right now for generic .c code.
         // Anyway the code cost is trivial, so it is fine to have on
         // even if it should never be used for MIPS.
         const char* useX11 = getenv( "DFB_USE_X11" );
         if ((useX11 != NULL) && (strcmp( useX11, "1" ) == 0))
         {
              dfb_config_set( "system", "x11" );
         }
         else
         {
              dfb_config_set( "system", "tivogfx" );
         }
     }
     #endif

     #ifdef TIVO
     if (getenv( "TIVO_DFB_USE_DEFAULT_STACK_SIZE" ) == NULL)
     {
         char s[ 32 ] = { 0 };
         snprintf( s, sizeof(s), "%d", 512 * 1024 /* TMK_MAX_STACK_SIZE */ );
         dfb_config_set( "thread-stacksize", s );
     }
     #endif
}
