...
static int s_processLog[128*1024];
static unsigned long s_processLogSizeBytes = sizeof(s_processLog);
...
{
    if (!monotonic_divisor) return 0ULL;
    return div64_u64((tivo_monotonic_retrieve(t) << 16), monotonic_divisor);
}
...
void _plog_diskstart_sata(struct request *rq)
{
    ...
    /* 
     * The use of rq->deadline parameter changed in kernel 2.6.37
...
static int __init plog_init(void)
{
    ...
    // matter.
    monotonic_divisor = div_u64((((u64) tivo_counts_per_256usec) << 8), 27);
