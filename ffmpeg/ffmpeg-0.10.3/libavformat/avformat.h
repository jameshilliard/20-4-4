...
/**
 * CHANGE LOG (TiVo):
 * 10/10/12: line 902-911
 *           Add segment_info struct, describing segment sequence number, duration.
 * 
 * 10/10/12: AVFormatContext, line 1191-1201.
 *           Add cb_chunk_start and cb_chunk_end callbacks to AVFormatContext.
 *
 * 03/15/13: AVFormatContext
 *           Add cb_stream_switch to notify of our switch to a new stream.
 */
...
/**
 * *** TiVo HLS ***
 * A struct that contains sequence number and duration for a given
 * segment in a stream.
 */
typedef struct {
    int seq_num;
...
#define RAW_PACKET_BUFFER_SIZE 2500000
    ...
    /**
     * *** TiVo HLS ***
     * Custom callbacks to notify upper layers of chunk boundaries.
     * @ cb_chunk_start.callback: A callback function that handles a chunk start.
     * @ cb_chunk_start.opaque: (Not used).
     *
     * @ cb_chunk_end.callback: A callback function that handles a chunk end.
     * @ cb_chunk_end.opaque: (Not used).
     */
    AVIOInterruptCB cb_chunk_start;
    AVIOInterruptCB cb_chunk_end;

    /**
     * *** TiVo HLS ***
     * Custom callback to notify upper layers that a switch has occured.
     */
    AVIOInterruptCB cb_stream_switch;
