...
static bool
#ifdef TIVO
expand_call_inline_guts
#else
expand_call_inline
#endif
(basic_block bb, tree stmt, tree *tp, void *data)
{
  copy_body_data *id;
  tree t;
  tree use_retvar;
...
#ifdef TIVO
// ABF: the following count is used by the tivoRefCount checking
//      to know when to ignore pushdecls.  (see bug #3802)
int expanding_inline_function = 0;
 
rtx
expand_call_inline (basic_block bb, tree stmt, tree *tp, void *data)
{
    rtx retval;
    expanding_inline_function++; // for tivoRefCount checking
 
    retval = expand_call_inline_guts (bb, stmt, tp, data);

    expanding_inline_function--; // for tivoRefCount checking
    /* assert( expanding_inline_function >= 0 ); */
