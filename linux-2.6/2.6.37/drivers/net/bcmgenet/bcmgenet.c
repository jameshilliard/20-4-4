...
/* Not making it static, since it needs to be accessed from prom.c */
u8 bcmgenet_cmdline_macaddr_eth[ETH_ALEN] ;
static u8 disable_eth = 0;
#endif
/* -------------------------------------------------------------------------
...
#ifdef CONFIG_PACESTB
	if ( (1 == disable_eth) && (0 == strcmp("eth0",dev->name)) )
		return -ENODEV;
#endif
...
#ifdef CONFIG_TIVO_GEN09
extern void mii_write(struct net_device *dev, int phy_id, int location, int val);
...
static void athsw_write_reg(struct net_device *dev, int addr, int data)
{
...
static void athsw_init(struct BcmEnet_devctrl *pDevCtrl)
{
    ...
    for (i = 0; i < 5; i++)
    {
        mii_write(dev, i, 0x1d, 0x3d);
        mii_write(dev, i, 0x1e, 0x48a0);
    }

    athsw_write_reg(dev, 0x0624, 0x007f7f7f);     // allow broadcast, multicast, unknown unicast to all ports

    // mac0 goes to the cpu
    athsw_write_reg(dev, 0x0004, 0x7800000);     // mac mode (external clock)
...
static int bcmgenet_drv_probe(struct platform_device *pdev)
{
	 ...
	 * connected we should get an interrupt and the operstate
...
static int bcmgenet_module_init(void)
{
	...
	if (boardID & kTivoConfigGen09Strap_IsBridge)
...
static int __init bcmgenet_disable_eth0(const char *val, struct kernel_param *kp)
{
	char x;

	if ( sscanf(val, "%c", &x) != 1 ) {
	  printk("%s: Failed to parse etherenet status", __FUNCTION__);
	return 0;
	}
	if((x == 'y') || (x == 'Y'))
	{
          printk("Force DISABLE eth0\n");
	  disable_eth=1;
	}
...
module_param_call(disable_eth0, bcmgenet_disable_eth0, NULL, NULL, 000);
MODULE_PARM_DESC(disable_eth0, "Disable eth0 "
		 "example brcmgenet.disable_eth0=y");
