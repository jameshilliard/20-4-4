...
static const StreamType HDMV_types[] = {
    ...
    /* *** TiVo - add 0x87 signature for certain EAC3 stream types*/
...
static const StreamType REGD_types[] = {
    ...
    /* *** TiVo - add EAC3 as a registration type, so it can
       be matched as a registration descriptor *** */
    { MKTAG('E','A','C','3'), AVMEDIA_TYPE_AUDIO, CODEC_ID_EAC3},
    /* *** TiVo *** */
    { 0 },
};
                              ...
                              MpegTSContext *ts)
{
    ...
    av_dlog(fc, "tag: 0x%02x len=%d, codec_id = %d stream_type=%d\n",
            desc_tag, desc_len, stream_type, st->codec->codec_id);
    ...
    case 0x05: /* registration descriptor */
        ...
        /* *** TiVo  * ***/
        /* Temporary fix to address a situation where certain
           HLS streams add a registration descriptor with EAC3 streams
           to probably unambiguously specify which audio standard they
           are using. This is temporary until we teach the HLS client
           code to learn to understand the codec_id
        */
            if (st->codec->codec_id == CODEC_ID_EAC3)
            {
                st->codec->codec_tag = stream_type;
            }
        /* *** TiVo *** */
        }
        break;
