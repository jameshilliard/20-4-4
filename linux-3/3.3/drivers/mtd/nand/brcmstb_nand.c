...
#ifdef CONFIG_PACESTB
    /* PACE gpio */
	int         		has_write_protect_gpio;
	int         		write_protect_gpio;
#endif
};
...
extern struct nand_ecclayout pace_brcmnand_oob_64;

/* PACE */
static void brcmnand_remove_write_protect_gpio( struct brcmstb_nand_host * host )
{
	if (host->hwcfg.has_write_protect_gpio)
	{
		/* Set the Not-write-protect pin high */
		gpio_set_level( host->hwcfg.write_protect_gpio, 1 );
	}
}

/* PACE */
static void brcmnand_apply_write_protect_gpio( struct brcmstb_nand_host * host )
{
	if (host->hwcfg.has_write_protect_gpio)
	{
		/* Set the Not-write-protect pin low */
		gpio_set_level( host->hwcfg.write_protect_gpio, 0 );
	}
}
#endif
...
#ifdef CONFIG_PACESTB
        brcmnand_remove_write_protect_gpio(host);
#endif
...
#ifdef CONFIG_PACESTB
	/* PACE Use Pace write protect/unprotect functions */
	if (dma_cmd == CMD_PAGE_READ)
  	   brcmnand_apply_write_protect_gpio( host );
	else
  	   brcmnand_remove_write_protect_gpio( host );
	...
	struct nand_chip *chip, u64 addr, const u32 *buf, u8 *oob)
{
	...
	bool restore_ecc_en = false;

	/* PACE fixup: if we want to just change the OOB bits, then
	   we have to put the device in ECC disable mode,
	   and restore ECC afterwards
	*/
	if (oob && !buf) {
		restore_ecc_en = true;
		ecc_en = RD_ACC_CONTROL(host->cs, WR_ECC_EN);
...
#ifdef CONFIG_PACESTB
	if (restore_ecc_en) {
		WR_ACC_CONTROL(host->cs, WR_ECC_EN, ecc_en);
	}
#else
	brcmstb_nand_wp(mtd, 1);
#endif
	...
	struct brcmstb_nand_cfg *cfg)
{
	...
	if (cfg->page_size < ARRAY_SIZE(page_sizes))
		...
		cfg->page_size = 2048;
#ifdef CONFIG_PACESTB
#if defined(CONFIG_MTD_BRCMNAND_HAS_WRITE_PROTECT_GPIO)
   	cfg->has_write_protect_gpio = 1;
    cfg->write_protect_gpio = CONFIG_MTD_BRCMNAND_WRITE_PROTECT_GPIO;
#else
  	cfg->has_write_protect_gpio = 0;
   	cfg->write_protect_gpio = 0;
#endif
    /* PACE Override these */
  	cfg->ecc_level = 15;
 	cfg->sector_size_1k = 0;
...
#ifndef CONFIG_PACESTB /* PACE removed this, keep using our OOB table */
