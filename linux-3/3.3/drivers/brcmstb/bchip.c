...
static void bchip_usb_init_one(int id, uintptr_t base)
{
	...
	/* power control setup */
#if !defined(CONFIG_TIVO_GEN10) && !defined(CONFIG_TIVO_GEN10)
...
#else
	// always explicitly configure usb
	if (boardID >= kTivoConfigBoardIDTitanRevCBase && boardID <= kTivoConfigBoardIDTitanRevCMax)
	{
		// titan IPP is active low 
		BDEV_SET(USB_REG(base, SETUP), USB_IPP);
	} else
	{
		// argon IPP is active high
		BDEV_UNSET(USB_REG(base, SETUP), USB_IPP);
	}    
	// over-current is active low
	BDEV_SET(USB_REG(base, SETUP), USB_IOC);
#endif        
