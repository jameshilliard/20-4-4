...
#ifdef CONFIG_TIVO
	/* Try to prevent cache line aliasing even if we don't have to. */
	if (current_cpu_data.dcache.linesz)
		shm_align_mask = max_t( unsigned long,
		    current_cpu_data.dcache.sets *
		    current_cpu_data.dcache.linesz - 1,
		    PAGE_SIZE - 1);
#endif
	...
	unsigned int cache)
{
	...
	if (cache & DCACHE) {
		...
		/* PACE - test this better safe than sorry approach */
		r4k_blast_dcache();
		r4k_blast_scache();
#else
...
#ifdef CONFIG_PACESTB
				/* PACE CHANGE - this used to bomb out returning -EFAULT */
				pg += PAGE_SIZE;
				continue;
#else
