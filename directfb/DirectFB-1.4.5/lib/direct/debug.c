                      ...
                      va_list            ap )
{
...
#ifdef TIVO
     {
         ...
         static bool sSimpleFormat = false;

         if (!sInit)
         {
             const char* dfbDebugSimpleOutput = getenv( "DFB_DEBUG_SIMPLE_OUTPUT" );
             if ((dfbDebugSimpleOutput != NULL) &&
                 (strcmp( dfbDebugSimpleOutput, "1" ) == 0))
             {
                 sSimpleFormat = true;
             }
         ...
         if (sSimpleFormat)
         {
             ...
             direct_log_printf( NULL, "(-) [%-15s] %s: %*s%s", thread ? thread : "  NO NAME",
                 name, indent, "", buf );
             return;
         }
     }
