...
static int ad_dispatch_requests(struct request_queue *q, int force)
{
	...
	*/
	...
	while (rq && (time_left > 0) ) {
	        ...
	        DPRINTK(" Adding: %lu: sector = 0x%llx, nr_sectors = 0x%lx, deadline = 0x%llx, est = %d\n", rq->start_time, 
	                rq->sector, rq->nr_sectors, rq->deadline, est);
...
/*        DPRINTK("\n"); */
	while (!list_empty(&sort_list) ) {
	         ...
	         */
	        DPRINTK(" Dispatching: %lu: sector = 0x%llx, nr_sectors = 0x%lx, deadline = 0x%llx\n", rq->start_time,
 ...
 */
static void *ad_init_queue(struct request_queue *q, struct elevator_queue *elevator)
{
	struct ad_data *dd;
