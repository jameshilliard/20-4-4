...
#if 0 /* PACE None of these are used but keep them here for reference */
/**
 * brcmnand_oob oob info for 512 page
 */
static struct nand_ecclayout brcmnand_oob_16 = {
	/* PACE - added 2 bytes at positions 14,15 for OOB ECC */
	.eccbytes	= 5,
	.eccpos		= {
		6,7,8,14,15
		},
	.oobfree	= { {.offset=0, .length=5}, 
				{.offset=9,.length=5}, /* Byte 5 (6th byte) used for BI */
...
#endif /* PACE - only use this in brcmnand_base.c */

/* PACE - added for the OOB ECC */

/* Enum used for the OOB Ecc correction logic */
enum BRCMNAND_OOBECC_STATUS {
   BRCMNAND_ECC_UNCORRECTABLE,
   BRCMNAND_ECC_FIXED,
   BRCMNAND_ECC_GOOD
} ;

extern void brcmnand_oobecc_calc( uint8_t* pui8Buffer, uint32_t ui32Length,
                                  uint8_t* pui8EccBuff ) ;
extern void brcmnand_oobecc_fix( uint8_t* pui8Buffer, uint32_t ui32Length,
