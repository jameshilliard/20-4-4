...
{
#ifdef CONFIG_TIVO
	/* This code uses semaphores for mutexes, so rather than fix it
	 * properly, just call sema_init() instead of init_MUTEX() that went
	 * away when real mutexes were implemented.
	 */
	sema_init((struct semaphore *)vmt, 1);
#else
...
#ifdef CONFIG_TIVO
DEFINE_SEMAPHORE (HostOS_thread_mutex);
#else
