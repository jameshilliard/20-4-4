...
#ifdef TIVO
/* Return 1 if this type is the enum TwStatus or TvStatus, 0 otherwise  */
static int tivoIsStatus( tree node )
{
  if ( TREE_CODE( node ) == REFERENCE_TYPE )
  {
    /* We don't audit for C++ reference return values */
    return 0;
  }
       ...
       TREE_CODE( node ) == INTEGER_TYPE)
  {
    ...
    if (TYPE_NAME (node))
    {
      if (TREE_CODE (TYPE_NAME (node)) == IDENTIFIER_NODE){
          str = IDENTIFIER_POINTER (TYPE_NAME (node));
      }
      else if (TREE_CODE (TYPE_NAME (node)) == TYPE_DECL
               && DECL_NAME (TYPE_NAME (node))){
        str = IDENTIFIER_POINTER (DECL_NAME (TYPE_NAME (node)));
      }
    }
 
    return ( strcmp( str, "TwStatus" ) == 0 ) ||
           ( strcmp( str, "TvStatus" ) == 0 );
  }
  else
  {
    return 0;
...
#ifdef TIVO
  /* If this is a function/method call that returns a
   * TvStatus/TwStatus and it is being implicitly ignored, warn if
   * TiVo warnings are enabled... */
  if (expr != error_mark_node
      && implicit
      && tivo_is_call_expr
      && warn_tivo
      ...
      && tivoIsStatus(TREE_TYPE(expr)))
  {
    warning (OPT_Wtivo, "(TiVo) status result of function call "
             "is ignored");
  }
#endif
