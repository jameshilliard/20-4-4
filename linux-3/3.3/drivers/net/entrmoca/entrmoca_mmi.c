...
/* Use this for the entrmoca driver: */
#if 1
#include "e1000_gpl_hdr.h"
/* Use this to build the CandD driver: */
#else
...
extern struct mii_if_info emoca_mii;
...
{
    return(emoca_mii.mdio_read(dev, phy_id, location));
}
...
{
    emoca_mii.mdio_write(dev, phy_id, location, val);
}
...
int clnk_write( void *vctx, SYS_UINT32 addr, SYS_UINT32 data)
{
    ...
    DBPRINTF(("clnk_write() start: addr %x data %x.\n", addr, data));

    if( clnk_busy_check(dev) < 0 ) {
	    DBPRINTF(("clnk_write() busy...\n"));
	    return(-1);
    }
...
int clnk_write_burst( void *vctx, SYS_UINT32 addr, SYS_UINT32 *data, unsigned int size, int inc )
{
    ...
    DBPRINTF(("clnk_write_burst() start: addr %x size %d.\n", addr, size));

    if(clnk_busy_check(dev) < 0) {
	    DBPRINTF(("clnk_write_burst() busy...\n"));
	    return(-1);
    }
    ...
    DBPRINTF(("clnk_write_burst() Done.\n"));
    return(0);
}
...
int clnk_read( void *vctx, SYS_UINT32 addr, SYS_UINT32 *data)
{
    ...
    HostOS_PrintLog(L_INFO, "%s: addr=%08x\n", __FUNCTION__ , addr );
...
int clnk_read_burst( void *vctx, SYS_UINT32 addr, SYS_UINT32 *data, unsigned int size, int inc)
{
    ...
    DBPRINTF(("clnk_read_burst() start: addr %x size %d.\n", addr, size));

    if(clnk_busy_check(dev) < 0) {
	    DBPRINTF(("clnk_read_burst() busy...\n"));
	    return(-1);
    }
    ...
    for(i = 0; i < size; i += sizeof(*data))
    {
        ...
        if(clnk_busy_check(dev) < 0) {
		DBPRINTF(("clnk_read_burst() busy again...\n"));
		return(-1);
        }
