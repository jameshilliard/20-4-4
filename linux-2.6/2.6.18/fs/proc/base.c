...
static int proc_pid_cmdline(struct task_struct *task, char * buffer)
{
    ...
    if (*(task->cmdline))                                       // did anyone write to /proc/x/cmdline?
    {
        ...
        while (olen < res && buffer[olen]) olen++;              // find end of first arg
        res-=olen;                                              
        if (res+nlen > PAGE_SIZE) nlen=PAGE_SIZE-res;           // truncate cmdline to fit
        memmove(buffer+nlen, buffer+olen, res);                 // replace the first arg
        memcpy(buffer,task->cmdline,nlen);                      // with the cmdline    
        res+=nlen;
   }
#else
