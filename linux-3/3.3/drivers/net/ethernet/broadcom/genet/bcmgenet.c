...
/* Additional definitions for activity LED control */
#define MII_AUXMODE2     0x1b      /* Aux Mode 2 MII register */
#define AUXMODE2_ACTLED  (1 << 5)  /* Bit to force activity LED on */
#endif
...
/* PACE - added to get MoCA and ethernet MAC address from kernel command line */
static u8 bcmgenet_cmdline_macaddr_moca[ETH_ALEN] ;
static u8 bcmgenet_cmdline_macaddr_eth[ETH_ALEN] ;
#endif
/* -------------------------------------------------------------------------
...
#ifdef CONFIG_PACE_BCMGENET_CABLE_DETECT
		/* Power-down the interface only if the cable detection interrupt flags have been enabled */
...
static int init_umac(struct BcmEnet_devctrl *pDevCtrl)
{
...
#ifdef CONFIG_PACE_BCMGENET_CABLE_DETECT
	pDevCtrl->cable_detect_interrupts_enabled = 0;
#endif /* #ifdef CONFIG_PACE_BCMGENET_CABLE_DETECT */
	...
	if (pDevCtrl->phyType == BRCM_PHY_TYPE_INT) {
		...
		intrl2->cpu_mask_clear |= (UMAC_IRQ_LINK_DOWN |
				UMAC_IRQ_LINK_UP);
#ifdef CONFIG_PACE_BCMGENET_CABLE_DETECT
		pDevCtrl->cable_detect_interrupts_enabled = 1;
#endif /* #ifdef CONFIG_PACE_BCMGENET_CABLE_DETECT */
...
#if defined(CONFIG_TIVO_GEN10)
extern int bcmgenet_mii_read(struct net_device *dev, int phy_id, int location);
extern void bcmgenet_mii_write(struct net_device *dev, int phy_id, int location, int val);
extern void athsw_write_reg(struct net_device *dev, int addr, int data);
extern int athsw_read_reg(struct net_device *dev, int addr);

static int athsw_read_hornet(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len=0, linkstat[1];
    struct net_device *dev = (struct net_device *)data;

    /* Get linkstat for ports 2 */
    linkstat[0] = (athsw_read_reg(dev, 0x0084) & (1<<8)) != 0;

    if (!off) 
	    len = sprintf(page, "%01d\n",
		linkstat[0]);
    /* page[len]=0; not needed. */
...
static int athsw_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len=0, linkstat[3];
    struct net_device *dev = (struct net_device *)data;

    /* Get linkstat for ports 3, 4, and 5 */
    linkstat[0] = (athsw_read_reg(dev, 0x0088) & (1<<8)) != 0;
    linkstat[1] = (athsw_read_reg(dev, 0x008c) & (1<<8)) != 0;
    linkstat[2] = (athsw_read_reg(dev, 0x0090) & (1<<8)) != 0;

    if (!off) 
	    len = sprintf(page, "%01d%01d%01d\n",
		linkstat[0],
		linkstat[1],
		linkstat[2]);
    /* page[len]=0; not needed. */
...
// Same as above, but each digit can be: 
// 0 = no link, 1 = 10HD, 2 = 10FD, 3 = 100HD, 4 = 100FD, 5 = 1000HD (impossible?), 6=1000FD
static int athsw_mode_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len=0, linkstat[3];
    int p;
    struct net_device *dev = (struct net_device *)data;
    ...
    for (p=0; p<3; p++)
    {
        int n=athsw_read_reg(dev, 0x88+(p*4));
        linkstat[p] = (n & 0x100)?((n&0x03)<<1)+((n&0x40)>>6)+1:0;    
    }
...
#define PORT0_LOOKUP_CTRL 0x660
#define PORT1_LOOKUP_CTRL 0x66C
#define PORT2_LOOKUP_CTRL 0x678
#define PORT3_LOOKUP_CTRL 0x684
#define PORT4_LOOKUP_CTRL 0x690
#define PORT5_LOOKUP_CTRL 0x69C
#define PORT6_LOOKUP_CTRL 0x6A8
#define PORT0_MOCA  0x1
#define PORT1_ZNVG  0x2
#define PORT2_WASP  0x4
#define PORT3_ETH3  0x8
#define PORT4_ETH1  0x10
#define PORT5_ETH2  0x20
#define PORT6_BCPU  0x40

extern unsigned int boardID;
static void athsw_init(struct BcmEnet_devctrl *pDevCtrl)
{
    struct net_device *dev = pDevCtrl->dev;
    int i;
    ...
    for (i = 0; i < 5; i++)
    {
        bcmgenet_mii_write(dev, i, 0x1d, 0x3d);
        //Aths switch fix
        bcmgenet_mii_write(dev, i, 0x1e, 0x68a0);
    }

    athsw_write_reg(dev, 0x0624, 0x007f7f7f);     // allow broadcast, multicast, unknown unicast to all ports

    // mac0 goes to the cpu 
    if (boardID & kTivoConfigGen10Strap_IsSwapBridge)
    {
        printk("Configuring port-6 to CPU\n");
        athsw_write_reg(dev, 0x000C, 0x5600000);     // mac mode (external clock)
        athsw_write_reg(dev, 0x0004, 0x1000000);     // mac mode (external clock)
        athsw_write_reg(dev, 0x0094, 0x0000007e);     // full duplex, flow control, rx and txmac, 1000M (RGMII)
        /*
         * Rule 1: Apart from Zenverge (P 1) & CPU (P 0), no one should forward packet to WASP(P 2)
         * Rule 2: Packets from WASP (P 2) should go only to Zvg (P 1) and CPU (P 0)
         */
        //Disable Port0 (MOCA) to forward packet to Port 2(WASP)
        uiValue = athsw_read_reg(dev, PORT0_LOOKUP_CTRL ); 
        uiValue = ((uiValue&0xFFFFFF80) | PORT1_ZNVG | PORT3_ETH3 | PORT4_ETH1 | PORT5_ETH2 | PORT6_BCPU);
        athsw_write_reg(dev,PORT0_LOOKUP_CTRL,uiValue);
        
        //Disable Port3 (Ethernet) to forward packet to Port 2(WASP)
        uiValue = athsw_read_reg(dev, PORT3_LOOKUP_CTRL ); 
        uiValue = ((uiValue &0xFFFFFF80) | PORT0_MOCA | PORT1_ZNVG | PORT4_ETH1 | PORT5_ETH2 | PORT6_BCPU);
        athsw_write_reg(dev,PORT3_LOOKUP_CTRL,uiValue);

        //Disable Port4 (Ethernet) to forward packet to Port 2(WASP)
        uiValue = athsw_read_reg(dev, PORT4_LOOKUP_CTRL ); 
        uiValue = ((uiValue &0xFFFFFF80) | PORT0_MOCA | PORT1_ZNVG | PORT3_ETH3|PORT5_ETH2 | PORT6_BCPU);
        athsw_write_reg(dev,PORT4_LOOKUP_CTRL,uiValue);

        //Disable Port5 (Ethernet) to forward packet to Port 2(WASP)
        uiValue = athsw_read_reg(dev, PORT5_LOOKUP_CTRL ); 
        uiValue = ((uiValue &0xFFFFFF80) | PORT0_MOCA | PORT1_ZNVG | PORT3_ETH3| PORT4_ETH1 | PORT6_BCPU );
        athsw_write_reg(dev,PORT5_LOOKUP_CTRL,uiValue);
        
        //Disable Port2(WASP) to forward packet to (ethernet) Port 3,4,5 and (Moca) Port 0 
        uiValue = athsw_read_reg(dev, PORT2_LOOKUP_CTRL ); 
        uiValue = ( (uiValue &0xFFFFFF80 ) | PORT1_ZNVG | PORT6_BCPU );
        athsw_write_reg(dev,PORT2_LOOKUP_CTRL,uiValue);

        //Zenverge should forward packets to all except to its own 
        uiValue = athsw_read_reg(dev, PORT1_LOOKUP_CTRL ); 
        uiValue = ((uiValue&0xFFFFFF80) | PORT0_MOCA | PORT2_WASP |PORT3_ETH3 | PORT4_ETH1 | PORT5_ETH2 | PORT6_BCPU);
        athsw_write_reg(dev,PORT1_LOOKUP_CTRL,uiValue);

        //CPU should forward packets to all except to its own 
        uiValue = athsw_read_reg(dev, PORT6_LOOKUP_CTRL ); 
        uiValue = ((uiValue&0xFFFFFF80) | PORT0_MOCA |PORT1_ZNVG| PORT2_WASP |PORT3_ETH3 | PORT4_ETH1 | PORT5_ETH2 );
        athsw_write_reg(dev,PORT6_LOOKUP_CTRL,uiValue);

    }
    else
    {
        printk("Configuring port 0 to CPU\n");
        athsw_write_reg(dev, 0x0004, 0x7600000);     // mac mode (external clock)
        athsw_write_reg(dev, 0x000C, 0x1000000);     // mac mode (external clock)
        athsw_write_reg(dev, 0x007c, 0x0000007e);     // full duplex, flow control, rx and txmac, 1000M (RGMII)
    }
    return;
}
#endif
...
static int bcmgenet_init_dev(struct BcmEnet_devctrl *pDevCtrl)
{
...
#if defined(CONFIG_TIVO_GEN10)
	// Configure the Atheros Ethernet switch
	...
	pDevCtrl->switchlinkstat = BMSR_LSTATUS; /* Assume on start the switch reports a link */
#endif
...
#if defined(CONFIG_TIVO_GEN10)
	/* XXX Do not stop the clock or MDIO won't work. */
		...
		struct ethtool_cmd *cmd)
{
	...
	if (pDevCtrl->phyType == BRCM_PHY_TYPE_MOCA) {
		...
		/* use autoneg to set Link status from the external switch */
		if (cmd->autoneg) {
			pDevCtrl->switchlinkstat = (BMSR_LSTATUS);
			pDevCtrl->irq0_stat &= ~UMAC_IRQ_PHY_DET_R;
			printk(KERN_CRIT "%s cable plugged in, powering up\n",
				pDevCtrl->dev->name);
			bcmgenet_power_up(pDevCtrl, GENET_POWER_CABLE_SENSE);
			...
			pDevCtrl->switchlinkstat = (BMSR_LSTATUS & 0x0000);
			pDevCtrl->irq0_stat &= ~UMAC_IRQ_PHY_DET_F;
			printk(KERN_CRIT "%s cable unplugged, powering down\n",
				pDevCtrl->dev->name);
			bcmgenet_power_down(pDevCtrl, GENET_POWER_CABLE_SENSE);
...
#ifdef CONFIG_TIVO_GEN10
	case BCMIOGSWREG: {
		struct athsw_data *mii_data = 
		    (struct athsw_data *)&rq->ifr_ifru;

		mii_data->val_out =
		    athsw_read_reg(dev, mii_data->val_in);
	}
                break;
		...
		athsw_write_reg(dev, mii_data->address, mii_data->val_in);
                break;
        }
#endif

#ifdef CONFIG_PACE_BCMGENET_CABLE_DETECT
	case SIOCSDETECTCABLESET:
		/* Set the cable in/out and link up/down detection interrupt flags by setting the appropriate bits to '1'
		   in the CPU_MASK_CLEAR register */
		spin_lock_irqsave(&pDevCtrl->lock, flags);
		...
		spin_unlock_irqrestore(&pDevCtrl->lock, flags);
		break;
	case SIOCSDETECTCABLEUNSET:
		/* Unset the cable in/out and link up/down detection interrupt flags by setting the appropriate bits to '1'
		   in the CPU_MASK_SET register */
		spin_lock_irqsave(&pDevCtrl->lock, flags);
		intrl2->cpu_mask_set |= (UMAC_IRQ_PHY_DET_R | UMAC_IRQ_PHY_DET_F);
		intrl2->cpu_mask_set |= (UMAC_IRQ_LINK_DOWN | UMAC_IRQ_LINK_UP);
		pDevCtrl->cable_detect_interrupts_enabled = 0;
		spin_unlock_irqrestore(&pDevCtrl->lock, flags);
		break;
	case SIOCSFORCECARRIERON:
		/* Force the carrier on */
		bcmgenet_power_up(pDevCtrl, GENET_POWER_CABLE_SENSE);
		netif_carrier_on(pDevCtrl->dev);
		pDevCtrl->dev->flags |= IFF_RUNNING;
		break;
	case SIOCSFORCECARRIEROFF:
		...
		break;
	case SIOCSLEDSTATE:
		/* Control the activity LED: normal operation or force on */
		auxregval = pDevCtrl->mii.mdio_read(pDevCtrl->dev, pDevCtrl->phyAddr, MII_AUXMODE2);
		if (rq->ifr_ifru.ifru_ivalue == 0)
			auxregval &= ~AUXMODE2_ACTLED;
		else
			auxregval |= AUXMODE2_ACTLED;
		pDevCtrl->mii.mdio_write(pDevCtrl->dev, pDevCtrl->phyAddr, MII_AUXMODE2, auxregval);
		break;
#endif /* #ifdef CONFIG_PACE_BCMGENET_CABLE_DETECT */
...
static int bcmgenet_drv_probe(struct platform_device *pdev)
{
	...
	struct net_device *dev;
        ...
        if (pdev->id)
        {
            printk("Ignoring genet probe instance %d\n", pdev->id);
            return -EIO;
        }
#endif        
...
#if defined (CONFIG_TIVO) || defined (CONFIG_PACESTB)
#ifdef CONFIG_NET_SCH_MULTIQ
        unsigned int queue_count = GENET_MQ_CNT+1;
#else
        unsigned int queue_count = 1;
#endif
#ifdef CONFIG_PACE_MOCADEV_NAME
        const char *fmt_string = cfg->phy_type == BRCM_PHY_TYPE_MOCA ? CONFIG_PACE_MOCADEV_NAME : "eth%d";   
#else
        const char *fmt_string = cfg->phy_type == BRCM_PHY_TYPE_MOCA ? "moca%d" : "eth%d";   
#endif
        dev = alloc_netdev_mq(sizeof(*(pDevCtrl)), fmt_string, ether_setup, queue_count);
...
#ifdef CONFIG_PACESTB
	/* PACE - modified to get MAC address from kernel command line */
	if ( cfg->phy_type == BRCM_PHY_TYPE_MOCA ) {
		memcpy(dev->dev_addr, bcmgenet_cmdline_macaddr_moca, 6);
	}
	else {
		memcpy(dev->dev_addr, bcmgenet_cmdline_macaddr_eth, 6);
	}
	/* PACE - print the MAC address */
	printk("%s MAC Address: %.2X:%.2X:%.2X:%.2X:%.2X:%.2X\n", CARDNAME,
...
#if defined(CONFIG_TIVO_GEN10)
		extern unsigned int boardID;
		/* If there's a switch the link is always up. */
		if (boardID & kTivoConfigGen10Strap_IsBridge) {
			pDevCtrl->dev->operstate = IF_OPER_UP;

			create_proc_read_entry("ar8328_hornet", 0, NULL,
			    athsw_read_hornet, dev);

			create_proc_read_entry("ar8328", 0, NULL,
			    athsw_read_proc, dev);

			create_proc_read_entry("ar8328_mode", 0, NULL,
			    athsw_mode_read_proc, dev);
		}
#endif
		/* No Link status IRQ */
...
#if defined(CONFIG_TIVO) && !defined(CONFIG_PACESTB)
		/* 
		 * Bug 310087 - Force the Ethernet link operstate to be DOWN
		 * during initialization, if the network cable is actually
		 * connected we should get an interrupt and the operstate will
		 * be set accordingly
		 */
                pDevCtrl->dev->operstate = IF_OPER_DOWN;
#endif
		bcmgenet_mii_setup(dev);
	}
...
static int __init bcmgenet_parse_mac_addr_moca(const char *val, struct kernel_param *kp)
{
	...
	for ( i = 0; i < 6; i++ ) {
		...
		bcmgenet_cmdline_macaddr_moca[i] = x;
		val += 2;
		if ( *val == ':' || *val == '-' ) {
			val++;
		}
	...
	if ( i != 6 ) {
		...
		memcpy(bcmgenet_cmdline_macaddr_moca, default_macaddr, sizeof bcmgenet_cmdline_macaddr_moca);
	}
        else if ( (0 == memcmp(bcmgenet_cmdline_macaddr_moca, invalid_macaddr[0], sizeof bcmgenet_cmdline_macaddr_moca)) ||
		  (0 == memcmp(bcmgenet_cmdline_macaddr_moca, invalid_macaddr[1], sizeof bcmgenet_cmdline_macaddr_moca)) )
	{
...
module_param_call(macaddr_moca, bcmgenet_parse_mac_addr_moca, NULL, NULL, 000);
MODULE_PARM_DESC(macaddr_moca, "MAC address for MoCA interface "
		 "example brcmgenet.macaddr_moca=00:c0:34:54:22:10");

/* PACE - added to get rear-panel ethernet MAC address from kernel command line */
static int __init bcmgenet_parse_mac_addr_eth(const char *val, struct kernel_param *kp)
{
        static u8 invalid_macaddr[2][6] = {
                { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
                { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff } };
        ...
        for ( i = 0; i < 6; i++ ) {
                ...
                bcmgenet_cmdline_macaddr_eth[i] = x;
                val += 2;
                if ( *val == ':' || *val == '-' ) {
                        val++;
                }
        ...
        if ( i != 6 ) {
                printk("%s: Failed to parse MAC address, using default value\n", __FUNCTION__);
                memcpy(bcmgenet_cmdline_macaddr_eth, default_macaddr, sizeof bcmgenet_cmdline_macaddr_eth);
        }
        else if ( (0 == memcmp(bcmgenet_cmdline_macaddr_eth, invalid_macaddr[0], sizeof bcmgenet_cmdline_macaddr_eth)) ||
                  (0 == memcmp(bcmgenet_cmdline_macaddr_eth, invalid_macaddr[1], sizeof bcmgenet_cmdline_macaddr_eth)) )
        {
                printk("%s: Invalid MAC address, using default value\n", __FUNCTION__);
                memcpy(bcmgenet_cmdline_macaddr_eth, default_macaddr, sizeof bcmgenet_cmdline_macaddr_eth);
        }
...
module_param_call(macaddr_eth, bcmgenet_parse_mac_addr_eth, NULL, NULL, 000);
MODULE_PARM_DESC(macaddr_eth, "MAC address for rear panel ethernet interface "
                 "example brcmgenet.macaddr_eth=00:c0:34:54:22:10");
#endif /* CONFIG_PACESTB */
