...
#ifdef LWS_OPENSSL_SUPPORT
	if (context->ssl_ctx && context->ssl_ctx_free)
...
/**
 * libwebsocket_get_ssl_common_name() - Retrieve the SSL common name of the
 *                                      peer.  If no valid client SSL cert,
 *                                      missing common name, or not using SSL
 *                                      mode, returns 1 and contents of name
 *                                      are undefined.  Otherwise returns 0.
 * @wsi:        Websocket connection instance to get SSL common name for
 * @name:       Preallocated buffer store the common name
 * @name_len:   Length of the name buffer
 */
int
libwebsocket_get_ssl_common_name(struct libwebsocket *wsi, char *name, int name_len)
{
    int rval = 1;
#ifdef LWS_OPENSSL_SUPPORT
    X509 *pPeer = NULL;
    if (wsi->ssl) {
        pPeer = SSL_get_peer_certificate(wsi->ssl);
        if (pPeer != NULL && (SSL_get_verify_result(wsi->ssl) == X509_V_OK)) {
            if (X509_NAME_get_text_by_NID(
                    X509_get_subject_name(pPeer),
                    NID_commonName,
                    /*returns*/name,
                    name_len) != -1) {
...
/**
 * libwebsocket_create_context_extended() - Create the websocket handler
 ...
 *			else ignored
 * @ssl_context: Preconstructed SSL_CTX * that already has the server certs
 *               and keys applied. Ownership remains with the caller, who is
 *               responsible for freeing it after the websockets context is
 *               destroyed.
 * @ssl_context_create_ifnull: Specifies whether SSL context should be created
 *                             if pre-configured one is not provided.
 * @gid:	group id to change to after setting listen socket, or -1.
 * @uid:	user id to change to after setting listen socket, or -1.
 * @options:	0, or LWS_SERVER_OPTION_DEFEAT_CLIENT_MASK
 * @userdata:   Private user data to associate with the context.  It is the
 *              responsibility of the caller to free this data when destroying
 *              the context.
 * 
...
libwebsocket_create_context_extended(int port, const char *interf,
			       struct libwebsocket_protocols *protocols,
			       struct libwebsocket_extension *extensions,
			       const char *ssl_cert_filepath,
			       const char *ssl_private_key_filepath,
                               void *ssl_context, int ssl_context_create_ifnull,
...
#ifdef LWS_OPENSSL_SUPPORT
	context->use_ssl = 0;
        context->ssl_ctx_free = 0;
	context->ssl_ctx = (SSL_CTX *) ssl_context;
...
//						context->canonical_hostname);
...
#ifdef LWS_OPENSSL_SUPPORT
                context->use_ssl = context->ssl_ctx != NULL ||
                    (ssl_cert_filepath != NULL &&
                     ssl_private_key_filepath != NULL);
		if (context->use_ssl)
			debug(" Compiled with SSL support, using it\n");
		else
                        debug("libwebsockets: Compiled with SSL support, "
                                "not using it\n");

#else
		if (context->ssl_ctx != NULL || ssl_cert_filepath != NULL
                    || ssl_private_key_filepath != NULL) {
			fprintf(stderr, "libwebsockets: Not compiled for OpenSSl support!\n");
			return NULL;
		}
		debug(" Compiled without SSL support, serving unencrypted\n");
#endif
	}
	...
	/* basic openssl init */
        ...
        // configure SSL context if one not provided
        if (ssl_context_create_ifnull && context->ssl_ctx == NULL)
        {
            SSL_library_init();
...
libwebsocket_create_context(int port, const char *interf,
			       ...
			       int gid, int uid, unsigned int options)
{
    return libwebsocket_create_context_extended(
        port, interf, protocols, extensions,
        ssl_cert_filepath, ssl_private_key_filepath, NULL, 1,
        gid, uid, options, NULL);
}


/**
 * libwebsocket_context_get_userdata() - Optional mechanism for retrieving
 *                              application-wide instance data stored
 *                              previously on the websocket context.  Returns
 *                              NULL if nothing has been stored.
 * @context:    server context returned by creation function
 */
LWS_EXTERN void *
libwebsocket_context_get_userdata(struct libwebsocket_context *context)
{
