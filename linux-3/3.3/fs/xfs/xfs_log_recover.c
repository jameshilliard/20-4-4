...
STATIC void 
xlog_dump_xlog_op_header(struct log *log, xlog_op_header_t *ohead)
{
	xfs_warn(log->l_mp, "PACE DEBUG: dumping xlog_op_header_t");
	xfs_warn(log->l_mp, "ohead @ %p", ohead);
	xfs_warn(log->l_mp, "ohead->oh_tid      = 0x%08X", ohead->oh_tid);
	xfs_warn(log->l_mp, "ohead->oh_len      = 0x%08X", ohead->oh_len);
	xfs_warn(log->l_mp, "ohead->oh_clientid = 0x%02X", ohead->oh_clientid);
	xfs_warn(log->l_mp, "ohead->oh_flags    = 0x%02X", ohead->oh_flags);
	xfs_warn(log->l_mp, "ohead->oh_res2     = 0x%04X", ohead->oh_res2);
}
#endif
...
#ifdef CONFIG_PACESTB
 			xlog_dump_xlog_op_header(log, ohead);
#endif
			ASSERT(0);
			return (XFS_ERROR(EIO));
		}
