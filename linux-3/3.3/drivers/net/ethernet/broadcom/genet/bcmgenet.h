...
struct BcmEnet_devctrl {
	...
	u32 switchlinkstat; /* Track the link status reported by the External Switch */
#endif
#ifdef CONFIG_PACE_BCMGENET_CABLE_DETECT
	/* Whether the cable in/out (UMAC_IRQ_PHY_DET_R, UMAC_IRQ_PHY_DET_F) and link up/down
	  (UMAC_IRQ_LINK_UP, UMAC_IRQ_LINK_DOWN) detection interrupt flags have been enabled */
	int cable_detect_interrupts_enabled;
