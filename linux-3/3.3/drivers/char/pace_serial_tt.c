 ...
 *********************************************************************
 *
 *  File        :  $Source: /home/sources/cvsroot/Thirdparty/siliconvendor/broadcom/nexus/kernel/stblinux-3.3/drivers/char/pace_serial_tt.c,v $
 *
...
{
   struct cdev          cdev;
   struct mutex         s_tt_lock;        /* Local lock replaces depricated use of kernel lock.. */
} S_TT_INSTANCE;
...
static long s_tt_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
   ...
   mutex_lock(&S_TT_Instance.s_tt_lock); 
   ...
   mutex_unlock(&S_TT_Instance.s_tt_lock); 
