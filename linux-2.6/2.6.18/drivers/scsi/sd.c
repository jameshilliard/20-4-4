...
#ifdef CONFIG_TIVO_NEUTRON
static int sd_start_stop(struct scsi_device *sdp, int start)
{
	int retries, res;
	struct scsi_sense_hdr sshdr;
...
static void sd_shutdown(struct device *dev)
{
	...
	if (sdkp->WCE) {
        ...
        if (boardID & kTivoConfigGen07Strap_HasISSD) {
            printk(KERN_NOTICE "Platform uses iSSD; stop drive %s ...\n", sdkp->disk->disk_name);
            sd_start_stop(sdp, 0);
        }
        else {
            printk(KERN_NOTICE "Platform doesn't use iSSD; don't stop drive %s\n", sdkp->disk->disk_name);
        }
#endif
	}
	scsi_disk_put(sdkp);
