			    ...
			    void __user *arg)
{
	...
	ata_id_string(dev->id, tempbuf, ATA_ID_PROD, ATA_ID_PROD_LEN);
	...
	memset(tempbuf, '\0', sizeof(tempbuf));
	memset(buf,'\0',sizeof(buf));
	ata_id_string(dev->id, tempbuf, ATA_ID_FW_REV, ATA_ID_FW_REV_LEN);

	/*trim the space characters at the end*/
	len = ATA_ID_FW_REV_LEN;
	while(isspace(tempbuf[--len]));
	...
	memset(tempbuf,'\0',sizeof(tempbuf));
	memset(buf,'\0',sizeof(buf));
	ata_id_string(dev->id, tempbuf, ATA_ID_SERNO, ATA_ID_SERNO_LEN);
