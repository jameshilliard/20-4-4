...
dfb_surface_buffer_lock_reset( CoreSurfaceBufferLock *lock )
{
     ...
     lock->phys       = 0;
#ifdef TIVO
     // fix compiler signed vs. unsigned
     lock->offset     = ~0UL;
#else
     lock->offset     = ~0;
#endif
     ...
     #define CORE_SURFACE_BUFFER_LOCK_ASSERT(lock)                                                       \
         do {                                                                                           \
              ...
              if ((lock)->buffer) {                                                                     \
                   ...
                   D_ASSUME( (lock)->addr != NULL || (lock)->phys != 0 || (lock)->offset != ~0UL || (lock)->handle != NULL );\
                   D_ASSUME( (lock)->offset == (lock)->allocation->offset || (lock)->offset == ~0UL );    \
