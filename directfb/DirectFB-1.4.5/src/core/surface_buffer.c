...
#ifdef TIVO
                    D_WARN( "Core/SurfBuffer: Buffer allocation failed!\n" );
#else
                         ...
                         const char        *prefix )
{
     ...
     if (alpha)
     {
         const char* omitAlphaDumpSpec = getenv( "DFB_DUMP_OMIT_ALPHA" );
         if ((omitAlphaDumpSpec != NULL) &&
             (strcmp( omitAlphaDumpSpec, "1" ) == 0))
         {
             //printf( "Dump omitting alpha...\n" );
             alpha = false;
         }
     }
#endif
