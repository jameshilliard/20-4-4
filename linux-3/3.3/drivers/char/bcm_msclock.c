...
/* The linux interrupt for the hardware timers.  This may be defined
 ...
 #define BCM_LINUX_TIMR_IRQ \
         (64 + BCHP_HIF_CPU_INTR1_INTR_W2_STATUS_UPG_TMR_CPU_INTR_SHIFT + 1)
#else
 #define BCM_LINUX_TIMR_IRQ \
...
static DEFINE_SPINLOCK(bcm_msclock_lock); 


/* Local ioctl lock */
static struct mutex bcm_msclock_ioctl_lock;
                              ...
                              unsigned long arg)
{
   ...
   mutex_lock(&bcm_msclock_ioctl_lock);
   ...
   mutex_unlock(&bcm_msclock_ioctl_lock);
...
static int __init bcm_msclock_init(void)
{
   ...
   */
   ...
   /* Initialise local ioctl lock */
   mutex_init(&bcm_msclock_ioctl_lock);
