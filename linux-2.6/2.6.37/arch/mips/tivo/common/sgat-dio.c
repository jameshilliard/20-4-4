...
#define getpage_address(x) lowmem_page_address(x)
#else
#define getpage_address(x) page_address(x)
#endif
...
#ifdef CONFIG_TIVO_PAC01
    gfp_mask &= ~(__GFP_HIGHMEM | __GFP_HIGH); /* Force allocations to low mem */
#endif
    maplist = kmalloc(wanted * sizeof(struct page **), gfp_mask);
	if (unlikely(!maplist))
		return -ENOMEM;
                                ...
                                sgat_dio_req *dio_req)
{
        ...
        for (k = 0, rem_sz = dxfer_len,iovec_count = 0,pgindex = 0;
                ...
                for( length = 0,ulen = useg_len,l = pgindex, toffset = offset,
                        ...
                        kaddress = (unsigned long) getpage_address(start_pg)+offset;
                        ...
                        if ( next_pg || (ulen / PAGE_SIZE)){
                                ...
                                toffset = 0;
                                if (next_pg && 
                                        ((unsigned long)getpage_address(cur_pg) + PAGE_SIZE) ==
                                        (unsigned long)getpage_address(next_pg))
                                        continue;
                        }
                        else {
                                /* Last page */
