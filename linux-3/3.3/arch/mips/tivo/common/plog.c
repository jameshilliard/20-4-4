 ...
 * 32-bits are used.  The timestamp is based on a 27 Mhz clock on all
 * platforms, which means that tools examining plog events can assume
...
/**
 * Convert a monotonic timestamp to a 27 Mhz base clock
 **/
static u64 monotonic_divisor;
static u64 monotonic_to_27Mhz(monotonic_t *t)
{
    return ((tivo_monotonic_retrieve(t) << 16) / monotonic_divisor);
}

/**
 * Retrieves the current monotime clock with a 27 Mhz base
 **/
static u64 now_27Mhz(void)
{
    monotonic_t mono_now;

    tivo_monotonic_get_current(&mono_now);

    return monotonic_to_27Mhz(&mono_now);
}
...
static int plog_assign_region(void)
{
    ...
    memset(s_processLog, 0, plog_size);
...
void _plog_diskdispatch(unsigned long start, struct request *req)
{
    ...
    if (req->bio)
    {
        buffer[2] = (int)bio_sectors(req->bio);
    }
    else
    {
        buffer[2] = 0;
...
void _plog_diskstart_sata(struct request *rq)
{
    ...
    /* 
     * The use of rq->deadline parameter changed in kernel 3.3
     * so we can't use it here to determine if the request
     * is real time.
     */
    buffer[1] = -1;
...
void _plog_diskstop_sata(struct request *rq, int write)
{
     ...
     * The use of rq->deadline parameter changed in kernel 3.3
     * so we can't use it here to determine if the request
     * is real time and calculate remaining time
     */
    buffer[1] = -1;
    buffer[3] = -1;
...
void _plog_queue_congested(int congested, int sync)
{
    int buffer[1];

    buffer[0] = sync;  // 0 - ASYNC WRITE, 1 - READ/SYNC WRITE

    _plLogEvent((congested == 1) ? PLOG_TYPE_QUEUE_CONGESTED : PLOG_TYPE_QUEUE_UNCONGESTED, sizeof(buffer), buffer);
}

void _plog_disk_request_failed_queue_full(int sync)
{
    int buffer[1];
...
void _plog_disk_request_failed_no_mem(int sync)
{
    int buffer[1];
...
void _plog_wake_up_queue(int sync)
{
    int buffer[1];
...
void _plog_add_disk_request_to_queue(struct request *rq)
{
    ...
    if (rq->bio)
        {
        buffer[3] = (int)bio_sectors(rq->bio);
        } 
        else 
        {
        buffer[3] = 0;
...
void _plog_disk_credited_writes(int n)
{
    int buffer[1];
...
void _plog_disk_repaid_credits(int n)
{
    int buffer[1];
...
static int __init plog_init(void)
{
    ...
    // Calculate the divisor necessary to get monotonic_t time to a 27 Mhz
    // clock base.  The top two bytes of the monotonic timestamp are thrown
    // away during the conversion by left shifting by 16 bits and then
    // dividing, in order to have the most efficient way to convert the clock
    // base while retaining the most accuracy possible.  This makes the clock
    // roll over every ~120 days (instead of never rolling over when no
    // conversion is done), but for plogging purposes, it's highly unlikely to
    // matter.
    monotonic_divisor = (((u64) tivo_counts_per_256usec) << 8) / 27;
